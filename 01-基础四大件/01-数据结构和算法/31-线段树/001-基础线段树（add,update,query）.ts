 ...
 * @LastEditTime: 2023-05-18 11:31:32
 * @FilePath: /algorithm/算法/31-线段树/001-基础线段树（add,update,query）.ts
...
class SegmentTree {
    ...
    // rt : 这个范围在sum中的下标
    public add(L: number, R: number, M: number, l: number, r: number, rt: number): void {
        //全包 懒住数据
        if (L <= l && r <= R) {
            this.lazy[rt] += M
            this.sum[rt] += (r - l + 1) * M
    ...
    public update(L: number, R: number, C: number, l: number, r: number, rt: number): void {
        if (L <= l && r <= R) {
