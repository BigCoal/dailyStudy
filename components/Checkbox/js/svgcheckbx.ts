...
export function draw(el: Element, type: BxType) {
  var paths: any = [],
    pathDef,
    animDef,
    svg = el.parentNode.querySelector(`.${type}-svg`);
...
export function controlCheckbox(el: any, type: BxType, svgDef?: Defs) {
...
function createSVGEl(def: Defs, type: BxType) {
  ...
  svg.setAttribute("class", type + "-svg");
  return svg;
}
